...
impl<'pctx, 'lxr, 'lctx> Parser {
    ...
    pub fn parse(iter: &'lxr mut Peekable<Chars<'lctx>>) -> Result<Expr, Error> {
    ...
    fn ty_checked_advance(&mut self, tk_ty: TokenType) -> Result<Token, Error> {
        match self.token_iter.next() {
            Some(token) => {
                let tok_span = token.get_span();
                self.hi = tok_span.base() + tok_span.len();
    ...
    fn consume_until<F: Fn(Token) -> bool>(&mut self, pred: F) -> Result<Expr, Error> {
    ...
    fn consume_line(&mut self) -> Result<Expr, Error> {
    ...
    fn build_ast_span(&mut self, lo: BytePos) -> Result<Span, Error> {
    ...
    fn load_symbols(&mut self, symbol: &Token, func: &Option<String>) -> Result<String, Error> {
        if let TokenType::Ident(ident) = symbol.peek_type() {
            if let Some(func_name) = func {
                // We are in a function local scope, first check to see if function already has locals vector.
                if let Some(locals) = self.function_symbols.get_mut(func_name) {
                    if locals.contains(&ident) {
                        self.consume_line()?;
                        return Err(Error::Redef(symbol.clone()));
                    }

                    // Function does not contain local, so add to list
                    locals.push(ident.clone());
                    Ok(ident)
                } else {
                    // Function does not yet contain symbols, so we are going to add some.
                    self.function_symbols
                        .insert(func_name.clone(), vec![ident.clone()]);
                    Ok(ident)
                }
            } else {
                // We are adding things to the global scope.
            ...
            self.consume_line()?;
            Err(Error::UxToken(String::from("ident"), symbol.clone()))
        }
    }

    fn check_symbol(&mut self, symbol: &Token, func: &Option<String>) -> Result<(), Error> {
        if let TokenType::Ident(ident) = symbol.peek_type() {
            if let Some(func_name) = func {
                if let Some(locals) = self.function_symbols.get(func_name) {
                    ...
                    match symbol.peek_type() {
                        TokenType::OutputNewLine | TokenType::OutputNum | TokenType::InputNum => {
                            return Ok(())
                        }
                        _ => {}
                    }
    ...
    pub fn build_ast(&mut self) -> Result<Expr, Error> {
    ...
    pub fn build_comp(&mut self) -> Result<Expr, Error> {
        ...
        Ok(Expr::Comp {
            globals,
            funcs,
            main: Box::new(main_body),
            span: self.build_ast_span(comp_lo)?,
        })
    }

    pub fn build_var_decl(&mut self, func: &Option<String>) -> Result<Vec<Expr>, Error> {
    ...
    pub fn build_var(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let var_lo = self.lo;
        return Ok(Expr::Var {
            idents: self.pull_idents_list(func)?,
            span: self.build_ast_span(var_lo)?,
        });
    }

    pub fn build_array(&mut self, func: &Option<String>) -> Result<Expr, Error> {
            ...
            match token.peek_type() {
                TokenType::LBracket => {
                    ...
                    if let TokenType::Number(_num) = depth.peek_type() {
                        ...
                        self.errors
                            .push(Error::UxToken(String::from("'[#number]'"), token));
                        return self
                            .consume_until(|tk| tk.peek_type() == TokenType::SemiTermination);
                    }
                    ...
                    if let TokenType::Ident(_) = self.peek()?.peek_type() {
                        return Ok(Expr::Array {
                            ...
                            idents: self.pull_idents_list(func)?,
                            span: self.build_ast_span(array_lo)?,
                        });
                    }
                }
                _ => {
                    self.errors
                        .push(Error::UxToken(String::from("'[', 'ident'"), token));
    ...
    fn pull_idents_list(&mut self, func: &Option<String>) -> Result<Vec<Expr>, Error> {
        ...
        loop {
            ...
            idents.push(Expr::Ident {
                val: token.clone(),
                span: self.build_ast_span(token.get_span().base())?,
            });
    ...
    fn build_func_decls(&mut self) -> Result<Vec<Expr>, Error> {
        ...
        loop {
            ...
            match token.peek_type() {
                TokenType::FuncDecl => {
                    ...
                    // TODO : Need and alternate method for this. Either by pre-pending func names to 
                    // be different from variables, or by maintaining a separate list.
                    ...
                    funcs.push(Expr::FuncDecl {
                        func_name: Box::new(Expr::Ident {
                            val: func_ident.clone(),
                            span: func_ident.get_span(),
                        }),
                        params: func_params,
                        var_decl: locals,
                        func_body: Box::new(body),
                        span: self.build_ast_span(func_lo)?,
                    })
                }
                _ => {
                    break;
    ...
    fn build_func_param(&mut self, func: &Option<String>) -> Result<Vec<Expr>, Error> {
    ...
    fn build_func_body(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let body_lo = self.lo;
        ...
        Ok(Expr::FuncBody {
            stmts,
            span: self.build_ast_span(body_lo)?,
        })
    }

    fn build_assignment(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        ...
        self.optional_end_take()?;
        Ok(Expr::Assign {
            design: Box::new(designator),
            expr: Box::new(expr),
            span: self.build_ast_span(assign_lo)?,
        })
    }

    fn optional_end(&mut self) -> Result<(), Error> {
        match self.peek()?.peek_type() {
            TokenType::Fi
            | TokenType::Od
            | TokenType::RBrace
            ...
            _ => Err(Error::UxToken(
                String::from("fi, od, '}', else, ';'"),
                self.peek()?,
            )),
        }
    }

    fn optional_end_take(&mut self) -> Result<(), Error> {
        match self.peek()?.peek_type() {
            TokenType::Fi
            | TokenType::Od
            | TokenType::RBrace
    ...
    fn build_if(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let if_lo = self.lo;
        ...
        if self.peek()?.peek_type() == TokenType::Else {
            ...
            self.optional_end_take()?;
            Ok(Expr::If {
                relation: Box::new(rel_op),
                if_body: Box::new(if_body),
                else_body: Some(Box::new(else_body)),
                span: self.build_ast_span(if_lo)?,
            })
        } else {
            self.ty_checked_advance(TokenType::Fi)?;
            self.optional_end_take()?;
    ...
    fn build_while(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        ...
        Ok(Expr::While {
            relation: Box::new(rel_op),
            func_body: Box::new(body),
            span: self.build_ast_span(while_lo)?,
        })
    }

    fn build_func_call(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let func_lo = self.lo;
        self.ty_checked_advance(TokenType::FuncCall)?;
        let ident = self.advance()?;
        match ident.peek_type() {
            TokenType::OutputNum | TokenType::OutputNewLine | TokenType::InputNum => { /* Pre-defined function, do nothing */
            }
    ...
    fn build_return(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let ret_lo = self.lo;
        ...
        Ok(Expr::Return {
            ...
            span: self.build_ast_span(ret_lo)?,
        })
    }

    fn build_designator(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let des_lo = self.lo;
        ...
        if self.peek()?.peek_type() == TokenType::LBracket {
            Ok(Expr::Design {
                ...
                exprs: self.build_array_expr_list(func)?,
                span: self.build_ast_span(des_lo)?,
            })
        } else {
            Ok(Expr::Design {
                ident: Box::new(Expr::Ident {
    ...
    fn build_array_expr_list(&mut self, func: &Option<String>) -> Result<Vec<Expr>, Error> {
        let mut expr_list = Vec::new();
        loop {
    ...
    fn build_expr(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let expr_lo = self.lo;
        ...
        match self.peek()?.peek_type() {
            TokenType::AddOp | TokenType::SubOp => {
                ...
                Ok(Expr::Expr {
                    l_expr: Box::new(lexpr),
                    r_expr: Some(Box::new(self.build_expr(func)?)),
                    math_op,
                    span: self.build_ast_span(expr_lo)?,
                })
            }
            _ => Ok(Expr::Expr {
                l_expr: Box::new(lexpr),
    ...
    fn build_term(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let term_lo = self.lo;
        ...
        match self.peek()?.peek_type() {
            TokenType::MulOp | TokenType::DivOp => {
                let math_op = Some(self.advance()?);
                Ok(Expr::Expr {
                    l_expr: Box::new(lfact),
                    r_expr: Some(Box::new(self.build_term(func)?)),
                    math_op,
                    span: self.build_ast_span(term_lo)?,
                })
            }
            _ => Ok(Expr::Expr {
                l_expr: Box::new(lfact),
    ...
    fn build_factor(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let factor = self.peek()?;
        match factor.peek_type() {
            TokenType::Ident(_) => self.build_designator(func),
            TokenType::Number(_) => Ok(Expr::Int {
                val: self.advance()?,
                span: factor.get_span(),
            }),
            ...
            _ => {
                self.consume_line()?;
                Err(Error::UxToken(
                    String::from("designator, number, function call, or '('"),
                    factor,
                ))
            }
        }
    ...
    fn build_relation(&mut self, func: &Option<String>) -> Result<Expr, Error> {
        let rel_lo = self.lo;
        ...
        Ok(Expr::Relation {
            l_expr: Box::new(lexpr),
            r_expr: Box::new(self.build_expr(func)?),
            rel_op: rel_op,
            span: self.build_ast_span(rel_lo)?,
        })
    }
}
