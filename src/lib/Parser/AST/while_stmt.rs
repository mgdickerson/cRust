...
impl WhileStmt {
    pub fn new(tc: &mut TokenCollection) -> Self {
            ...
            .get_type()
        {
            ...
            err => {
                // Compiler Error :
                panic!(
                    "Expected While statement, found unexpected Token: {:?}",
                    err
                );
            }
        }
            ...
            Some(TokenType::OdStatement) => {
                ...
                match tc.peek_next_token_type() {
                    ...
                    None => {
                        // Compiler Error :
                        panic!(
                            "End of file found, do should be appended by '}' if end of statement"
                        );
                    }
                    err => {
                        // Compiler Error :
        ...
        WhileStmt {
            node_type: TokenType::WhileStatement,
            relation,
            body,
        }
    }
    ...
    pub fn to_ir(self, irgm: &mut IRGraphManager) {
        ...
        irgm.graph_manager()
            .switch_current_node_index(loop_header.clone());
        ...
        let bra_return =
            irgm.build_op_y(Value::new(ValTy::node_id(loop_header.clone())), InstTy::bra);
        ...
        irgm.graph_manager()
            ...
            .unwrap()
            .borrow_mut()
            .update_y_val(Value::new(ValTy::node_id(branch_node)));
