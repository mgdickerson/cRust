...
impl ArrayManager {
    pub fn new() -> Self {
        ArrayManager {
            array_manager: HashMap::new(),
            array_global: Vec::new(),
            active_func: None,
        }
    }
    ...
    pub fn add_array(&mut self, array_ident: &String, array_depth: Vec<Number>) {
        self.array_manager.insert(
            array_ident.clone(),
            UniqueArray::new(array_ident.clone(), array_depth, None),
        );
    }
    ...
    pub fn assign_addr(&mut self, array_ident: String, uniq_addr: UniqueAddress) {
        self.array_manager
            .get_mut(&array_ident)
            .expect("array should exist before assigning address.")
            .assign_address(uniq_addr);
    }

    pub fn get_array_ref(&self, array_ident: String) -> &UniqueArray {
        match self.array_manager.get(&array_ident) {
            Some(arr_ident) => arr_ident,
    ...
    pub fn build_inst(
        ...
        uniq_arr: UniqueArray,
        val_vec: Vec<Value>,
        value_to_assign: Option<Value>,
    ) -> Value {
        let mut last_offset_inst: Option<Value> = None;

        val_vec.iter().enumerate().for_each(|(adjust, val)| {
            ...
            if let ValTy::con(val_con) = val.get_value().clone() {
                ...
                match last_offset_inst.clone() {
                    Some(last_inst) => {
                        if let ValTy::con(last_con) = last_inst.clone().get_value() {
                            ...
                            let offset_inst =
                                irgm.build_op_x_y(last_inst, new_adjust_value, InstTy::add);
                ...
                let adjustment = Value::new(ValTy::con(adj_size.clone()));
        ...
        let final_offset;
        if let ValTy::con(final_con) = last_offset_inst
            .clone()
            .expect("Should be at least one instruction")
            .get_var_base()
        {
            let add_load = irgm.build_op_x_y(
                Value::new(ValTy::con(0)),
                last_offset_inst.unwrap(),
                InstTy::add,
            );
        ...
        match &value_to_assign {
            Some(val) => {
                ...
                if let ValTy::con(val_store) = val.get_value().clone() {
                    let add_inst =
                        irgm.build_op_x_y(Value::new(ValTy::con(0)), val.clone(), InstTy::add);
...
impl UniqueArray {
    pub fn new(
        arr_ident: String,
        arr_depth: Vec<Number>,
        uniq_addr: Option<UniqueAddress>,
    ) -> Self {
        let array_depth = arr_depth
            .iter()
            .map(|num| num.get_value())
            .collect::<Vec<i32>>();
        UniqueArray {
            base_ident: arr_ident,
            array_depth,
            uniq_addr,
        }
    }
    ...
    pub fn clone_addr(&self) -> UniqueAddress {
        self.uniq_addr
            .clone()
            .expect("Address not available to clone")
    }
